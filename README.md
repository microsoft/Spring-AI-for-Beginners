...
> **Models used:** `azd up` provisions two model deployments — one chat model and one embedding model. Every module uses **gpt-5.6-luna**, a reasoning model, so demos stay fast and the focus stays on the Spring AI patterns; module 02 additionally varies its **reasoning effort** to show how thinking budget changes the answer, and module 03 uses **text-embedding-3-small** for RAG. The deployments are routed via env vars (`AZURE_OPENAI_DEPLOYMENT` / `AZURE_OPENAI_FAST_DEPLOYMENT` / `AZURE_OPENAI_EMBEDDING_DEPLOYMENT`) — see [01-introduction/infra/README.md](01-introduction/infra/README.md) for details.
